...
nbd=${SHEDOS_BASE_NBD:-/dev/nbd0}

_die() { echo "build-base-image: $1" >&2; exit 1; }
[[ $EUID -eq 0 ]] || _die "must run as root (nbd + chroot)"
[[ -n $iso || -n $airootfs_dir ]] || _die "set SHEDOS_ISO or SHEDOS_AIROOTFS_DIR"
for t in qemu-img qemu-nbd sgdisk mkfs.btrfs mkfs.fat arch-chroot unsquashfs blkid; do
...
echo "build-base-image: creating $out ($size)"
...
ok = LimineInstaller(mount_point=mount, root_uuid=root_uuid).install(disk)
...
sync
echo "build-base-image: detaching"
cleanup
trap - EXIT
